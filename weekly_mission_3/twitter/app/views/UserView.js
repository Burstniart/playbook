...
class UserView {
    static createUser(payload) {
        if(payload === null) {
            ...
            return {error : 'must have valid values'}
        }
    }
}
...
// Requerimientos:
// 1. Valida que al enviar en el `payload` un valor `null`, obtenga un objeto con la llave `error` y el valor 
// indique `payload no existe`.
// 2. Valida que un `payload` contenga en alguna de las llaves `username`, `name`, o `id` un valor en `null`.
//  Si hay un valor `null` regresa un objeto con la llave `error` y que indique el texto: `necesitan tener un 
//  valor válido`.
